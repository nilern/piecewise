...
use gce::layout::{Block, Granule, GSize};
use gce::descriptor::Descriptor;
...
impl ValueRef {
    ...
    pub fn ptr(self) -> Option<Shared<Object>> {
        if self.0 & Self::PTR_BIT == 1 {
            Shared::new((self.0 & !Self::TAG_MASK) as *mut Object)
        } else {
            None
        }
    }
...
impl From<Shared<Object>> for ValueRef {
    fn from(ptr: Shared<Object>) -> ValueRef {
        ValueRef(unsafe { ptr.as_ptr() } as usize | Self::PTR_BIT)
    }
}

impl From<Shared<PointyObject>> for ValueRef {
    fn from(ptr: Shared<PointyObject>) -> ValueRef {
        ValueRef(unsafe { ptr.as_ptr() } as usize | Self::POINTY_BIT | Self::PTR_BIT)
    }
}
